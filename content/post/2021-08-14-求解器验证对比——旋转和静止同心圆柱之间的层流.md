...
![20210814-1-1.png](/resources/20210814-1-1.png)
...
![20210814-2-1.png](/resources/20210814-2-1.png)

导入网格模型VMFL001_rot_conc_cyl_2D.msh，如下所示。

![20210814-2-2.png](/resources/20210814-2-2.png)

更改物理模型，选择层流。

![20210814-2-3.png](/resources/20210814-2-3.png)

更改流体参数。将"air"重命名为"vmfl001"，密度1kg/m3，粘度0.0002kg/m-s。

![20210814-2-4.png](/resources/20210814-2-4.png)

![20210814-2-5.png](/resources/20210814-2-5.png)
...
![20210814-2-6.png](/resources/20210814-2-6.png)

求解器选择SIMPLE，如下所示。

![20210814-2-7.png](/resources/20210814-2-7.png)

收敛残差更改为1e-6，初始化。

![20210814-2-8.png](/resources/20210814-2-8.png)

![20210814-2-9.png](/resources/20210814-2-9.png)

迭代步数800，求解并保存。

![20210814-2-10.png](/resources/20210814-2-10.png)

![20210814-2-11.png](/resources/20210814-2-11.png)
...
![20210814-2-12.png](/resources/20210814-2-12.png)

![20210814-2-13.png](/resources/20210814-2-13.png)

输出四个点的切向速度。

![20210814-2-14.png](/resources/20210814-2-14.png)

新建一条水平方向的直线，输出不同半径下的切向速度分布曲线。

![20210814-2-15.png](/resources/20210814-2-15.png)

![20210814-2-16.png](/resources/20210814-2-16.png)

![20210814-2-17.png](/resources/20210814-2-17.png)

将曲线点数据输出到文件中备用。

![20210814-2-18.png](/resources/20210814-2-18.png)
...
![20210814-3-1.png](/resources/20210814-3-1.png)

新建材料，命名为"vmfl001"。密度1 [kg m^-3]，动力粘度0.0002 [Pa s]。

![20210814-3-2.png](/resources/20210814-3-2.png)

新建流体域（Domain），命名为FLUID，网格区域（Location）选择FLUID。材料选择刚刚新建的vmfl001。关闭传换热方程，湍流方程选择None(Laminar)。

![20210814-3-3.png](/resources/20210814-3-3.png)

![20210814-3-4.png](/resources/20210814-3-4.png)
...
![20210814-3-5.png](/resources/20210814-3-5.png)

新建交界面（Interfaces），命名为PERIODIC，如下图所示。

![20210814-3-6.png](/resources/20210814-3-6.png)

新建全局初始化（Global Initialization），直接确认，采用默认值即可。

![20210814-3-7.png](/resources/20210814-3-7.png)

更改求解器控制（Solver Control），最大迭代步数改为1000，残差类型改为MAX，收敛残差目标改为1e-6。

![20210814-3-8.png](/resources/20210814-3-8.png)

保存算例，输出.def文件。打开CFS-Solver Manager载入.def文件，求解器类型选择双精度，开始求解。

![20210814-3-9.png](/resources/20210814-3-9.png)

大概23步左右就收敛了。

![20210814-3-10.png](/resources/20210814-3-10.png)
...
![20210814-3-11.png](/resources/20210814-3-11.png)

建立直线"Line 1"，如下图所示。

![20210814-3-12.png](/resources/20210814-3-12.png)
...
![20210814-3-13.png](/resources/20210814-3-13.png)
...
![20210814-4-1.png](/resources/20210814-4-1.png)

选择求解器物理模型，如下图所示。

![20210814-4-2.png](/resources/20210814-4-2.png)

将气体材料"Air"重命名为"vmfl001"，密度修改为1.0 kg/m^3，动力粘度修改为2.0E-4 Pa-s。

![20210814-4-3.png](/resources/20210814-4-3.png)

展开流体区域中的"FLUID-7"，同时选中"PERIODIC"和"SHADOW_PERIODIC"边界，右键创建界面。新建的交界面拓扑改为"周期"，周期转换选择"旋转"，旋转轴指定改为"指定轴"，轴的方向为"[0.0, 0.0, 1.0]"。

![20210814-4-4.png](/resources/20210814-4-4.png)

在"工具"、"运动"中新建旋转运动，旋转轴方向[0.0, 0.0, 1.0]，旋转速率1.0 radian/s。

![20210814-4-5.png](/resources/20210814-4-5.png)

将INNERWALL边界的参考坐标系指定选项更改为"局部参考坐标系"，边界参考坐标系指定选择刚刚新建的旋转运动的坐标系。

![20210814-4-6.png](/resources/20210814-4-6.png)

停止标准中的最大迭代步数默认为1000，运行求解并保存算例。

![20210814-4-7.png](/resources/20210814-4-7.png)

### 4.2 后处理

在衍生零部件中新建4个探针点，如下表。

![20210814-4-8.png](/resources/20210814-4-8.png)
...
![20210814-5-1.png](/resources/20210814-5-1.png)

以vmfl001.foam为基础，新建4个"Probe Location"，用于提取20~35mm处的切向速度。

![20210814-5-2.png](/resources/20210814-5-2.png)
...
![20210814-5-3.png](/resources/20210814-5-3.png)

选中该GroupDatasets，可在右侧"SpreadSheetView"窗口中看到4个点位置的切向速度值。

![20210814-5-4.png](/resources/20210814-5-4.png)

以vmfl001.foam为基础，建立"PlotOnIntersectionCurves"绘图。

![20210814-5-5.png](/resources/20210814-5-5.png)

平面方向选择Y轴正向，如图所示。

![20210814-5-6.png](/resources/20210814-5-6.png)

X轴显示变量选择"Points_X"，Y轴显示变量选择"U_Y"，如图所示。

![20210814-5-7.png](/resources/20210814-5-7.png)

右侧"LineChartView"窗口可以查看生成的曲线。

![20210814-5-8.png](/resources/20210814-5-8.png)
...
![20210814-6-1.png](/resources/20210814-6-1.png)
