+++
author = "Andrew Moa"
title = "A Qt6-based image base64 conversion tool"
date = "2025-04-02"
description = ""
tags = [
    "c++",
...
Tested with PNG format, this tool successfully renders images in [Marktext](https://github.com/marktext/marktext) and [Joplin](https://joplinapp.org/). Other formats depend on the Markdown editor's rendering engine. For screenshots, PNG is recommended.
...
Image display is implemented through QLabel, which overloads the QLabel class[^2] and makes some adjustments.
...
It is worth noting that in the Python implementation code, the conversion of binary data to base64 is completed through Python's str function, so the output string will contain the label `b'...'`, which needs to be removed through string slicing.
`widget.py`：
```python
